...
def default_fproc(
    *, op, a_layout, b_layout, c_layout, dtype, epilogue_name, permute_layout=None
    ...
    if (
        ...
        # EmitGemmUniversal3xInstance.emit BEFORE _replace_epilogue_cutlass_3x swaps in the
        # real residual fusion. The functor here is only a placeholder for the emitter
        # (the broadcast path re-derives the epilogue from the op type), so map it to the
        # plain 3x LinearCombination too; the residual add is restored by the epilogue
        # replacement + the ptr_C0/beta=1 problem_args.
        if op.gemm_kind == cutlass_lib.library.GemmKind.Universal3x and op.epilogue_functor in (
            cutlass_lib.library.EpilogueFunctor.LinearCombination,
