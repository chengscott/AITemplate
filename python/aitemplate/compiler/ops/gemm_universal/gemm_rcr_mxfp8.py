...
class gemm_rcr_mxfp8(Operator):
    ...
    def __call__(self, a, b, b_scale, residual=None,
        ...
        # a [M,K] f16 quantized to e4m3+SFA internally. norm='rmsnorm' folds RMSNorm(a)*gamma
        # (+relu) into that internal quantize; norm='swiglu' folds SwiGLU (a is the packed fc1
        # [M,2*ffn]; z=silu(rrms*gate)*(rrms*up)); norm='rms_out' folds ops.rms_reduce (quantizes
        # RAW a, emits rrms as a 2nd output) -- all mega-fused (no separate norm/swiglu kernel +
        # f16 round-trip). The SFA stays a STATIC workspace (no dynamic SF graph tensor). b [N,K]
        # e4m3, b_scale ue8m0 SFB (swizzled, baked at export). K % 32 == 0. In swiglu mode the
        # gemm K = ffn = a's last dim // 2.
